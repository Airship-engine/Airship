...
TEST(Logging, fileoutput) {
...

    {
        ...
        if constexpr (Airship::ShipLog::IsLevelEnabled(Airship::ShipLog::Level::ERROR))
            ASSERT_NE(fileContents.str().find("Logging an error"), std::string::npos)
                << "File: " << filename << "\nFile Contents: \"" << fileContents.str() << "\"";
        else
            ASSERT_EQ(fileContents.str().find("Logging an error"), std::string::npos)
                << "File: " << filename << "\nFile Contents: \"" << fileContents.str() << "\"";
    }
...

    {
        ...
        ASSERT_EQ(fileContents.str().find("This is a debug message"), std::string::npos)
            << "File: " << filename << "\nFile Contents: \"" << fileContents.str() << "\"";
    }

...
Input::Key GLFWKeyToAirship(int key) {
    switch (key) {
        ...
        default: 
            SHIPLOG_ERROR("Invalid key press: {}", key);
        // clang-format on
    }
...
Input::KeyMods GLFWKeyModsToAirship(int mods) {
    Input::KeyMods ret = Input::KeyMods::None;
    if ((mods & GLFW_MOD_SHIFT) == GLFW_MOD_SHIFT) ret |= Input::KeyMods::Shift;
    if ((mods & GLFW_MOD_CONTROL) == GLFW_MOD_CONTROL) ret |= Input::KeyMods::Ctrl;
    if ((mods & GLFW_MOD_ALT) == GLFW_MOD_ALT) ret |= Input::KeyMods::Alt;
    if ((mods & GLFW_MOD_SUPER) == GLFW_MOD_SUPER) ret |= Input::KeyMods::Super;
    return ret;
}

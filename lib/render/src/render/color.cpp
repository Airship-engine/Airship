...
RGBColor blendAlpha(const RGBColor& bg, const RGBColor& fg) {
    ...
    return {(fg.r * fgScale) + (bg.r * bgScale), (fg.g * fgScale) + (bg.g * bgScale),
            (fg.b * fgScale) + (bg.b * bgScale), newA};
}
...
RGBColor RGBColor::blend(const RGBColor& bg, const RGBColor& fg, RGBColor::BlendMode mode) {
    switch (mode) {
    case RGBColor::BlendMode::Alpha:
        return blendAlpha(bg, fg);
    case RGBColor::BlendMode::Multiply:
        return blendMultiply(bg, fg);
    case RGBColor::BlendMode::Add:
        return blendAdd(bg, fg);
    case RGBColor::BlendMode::Average:
        return blendAverage(bg, fg);
    }
    assert(false);
    return ERR_COLOR;
}
...
RGBColor RGBColor::normalize(RGBColor::NormalizeMode mode) const {
    switch (mode) {
    case RGBColor::NormalizeMode::Clamp:
        return normalizeClamp(*this);
    case RGBColor::NormalizeMode::Scale:
        return normalizeScale(*this);
    }
    assert(false);
    return ERR_COLOR;
}
